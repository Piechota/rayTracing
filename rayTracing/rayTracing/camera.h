...
public:
	CCamera( Vec3 const lookFrom, Vec3 const lookAt, Vec3 const up, unsigned int const width, unsigned int const height, float const vfov, float const aperture, float const focusDistance )
	{
	...
	CRay GetRay(float const s, float const t ) const
	{
		...
		return CRay( m_origin + offset, m_bottomLeftCorner + m_horizontal * s + m_vertical * t - m_origin - offset );
	}
};
